...
class SendToken extends Component {
  ...
  showSendScreen = () => {
    ...
    return data.stepsConfig.map(config => {
      let val = Object.values(config)[0];
      return (<div className="custom-control custom-checkbox m-3" style={{color: '#4d5d71'}}>
      <input type="checkbox" class="custom-control-input" id="customCheck" name={Object.keys(config)[0]} defaultChecked={true}  onChange={this.handleInputChange} />
      <label class="custom-control-label" for="customCheck">{val.msg}</label>
    </div>);
    });
  };
